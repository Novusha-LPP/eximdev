...
const DocumentTable = ({ docs, fields }) => {
  ...
  const formatValue = (value, key) => {
    ...
    if (
      (key === "url" ||
        key === "document_charge_recipt_copy" ||
        key === "payment_recipt") &&
    ...
    if (
      typeof value === "string" &&
      (key.endsWith("date") || key.includes("date"))
    ) {
  ...
  const applyBusinessRules = (fields, docs) => {
    ...
    return fields.filter((field) => {
      ...
      const hasData = validDocs.some(
        (doc) => doc[field.key] !== undefined && doc[field.key] !== null
      );
              ...
              {relevantFields.map((field) => (
                <TableCell
                  ...
                  sx={{
                    ...
                    whiteSpace:
                      field.key === "document_name" ? "normal" : "nowrap",
                    fontSize: "0.8rem",
                    py: 1.5,
                  }}
                >
...
const Charges = ({ job_no, year }) => {
  ...
  const fetchCharges = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_API_STRING}/charges-section/job-details`,
        {
  ...
  if (!job_no || !year) {
    return (
      <Alert severity="warning" sx={{ borderRadius: 2, m: 2 }}>
        <Typography>
          Job Number and Year are required to fetch charges details.
        </Typography>
      </Alert>
    );
  }
  ...
  const formatBondChargesForTable = (data) => {
    ...
    if (
      ...
      !data.shipping_line_bond_valid_upto &&
      (!data.shipping_line_bond_docs ||
        data.shipping_line_bond_docs.length === 0)
    ) {
  ...
  return (
    <Box sx={{ p: 2, backgroundColor: "#f8fafc", minHeight: "100vh" }}>
      ...
      >
        <DocumentTable
          docs={formatBondChargesForTable(data)}
          fields={shippingLineBondFields}
        />
      </Section>
    </Box>
  );
