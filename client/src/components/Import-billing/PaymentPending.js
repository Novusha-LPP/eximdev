        ...
        <DialogContent sx={{ p: 0, backgroundColor: '#fff' }}>
          {isModalLoading ? <Box display="flex" justifyContent="center" p={4}><CircularProgress size={40} /></Box> : selectedPaymentRequest && (
            <Box sx={{ p: 2 }}>
              ...
              {selectedPaymentRequest.items && selectedPaymentRequest.items.length > 0 && (
                <Box sx={{ mb: 2, border: '1px solid #ccc' }}>
                  <Box sx={{ backgroundColor: '#f5f5f5', p: 1, borderBottom: '1px solid #ccc' }}>
                    <Typography variant="caption" fontWeight="bold">INCLUDED CHARGES ({selectedPaymentRequest.items.length})</Typography>
                  </Box>
