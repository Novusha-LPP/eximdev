...
function Penalty() {
  ...
  const fetchPenaltyData = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_STRING}/report/penalty`
      );
  ...
  const totalInterest = data.reduce(
    (sum, job) => sum + (parseFloat(job.intrest_ammount) || 0),
    0
  );
  const totalFines = data.reduce(
    (sum, job) => sum + (parseFloat(job.fine_amount) || 0),
    0
  );
  const totalPenalties = data.reduce(
    (sum, job) => sum + (parseFloat(job.penalty_amount) || 0),
    0
  );
  ...
  const filteredData = data.filter((job) => {
    ...
    const matchesImporter =
      !selectedImporter || job.importer === selectedImporter;
                        ...
                        <TableCell align="right" sx={{ py: 2 }}>
                          <Typography
                            ...
                            sx={{
                              fontWeight: 600,
                              color:
                                jobTotal > 0 ? "error.main" : "text.secondary",
                            }}
                          >
