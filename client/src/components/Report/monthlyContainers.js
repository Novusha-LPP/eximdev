...
const TrendChart = ({ trendData }) => {
  ...
  const dataWithTEU = trendData.map((item) => ({
    ...item,
    netContainer20Ft: Math.max(
      0,
      (Number(item.container20Ft) || 0) - (Number(item.lcl20Ft) || 0)
    ),
    netContainer40Ft: Math.max(
      0,
      (Number(item.container40Ft) || 0) - (Number(item.lcl40Ft) || 0)
    ),
    teu: Math.max(
      0,
      (Number(item.container20Ft) || 0) -
        (Number(item.lcl20Ft) || 0) +
...
const MonthlyContainers = () => {
  ...
  const generateMockTrendData = (row) => {
    ...
    return months.map((month, index) => {
      ...
      return {
        month,
        beDateCount: Math.max(
          1,
          Math.round(
            currentValues.beDateCount * (1 + (variation * (index + 1)) / 6)
          )
        ),
        container20Ft: Math.max(
          0,
          Math.round(
            currentValues.container20Ft * (1 + (variation * (index + 1)) / 6)
          )
        ),
        container40Ft: Math.max(
          0,
          Math.round(
            currentValues.container40Ft * (1 + (variation * (index + 1)) / 6)
          )
        ),
        lcl20Ft: Math.max(
          0,
          Math.round(
            currentValues.lcl20Ft * (1 + (variation * (index + 1)) / 6)
          )
        ),
        lcl40Ft: Math.max(
          0,
          Math.round(
            currentValues.lcl40Ft * (1 + (variation * (index + 1)) / 6)
          )
        ),
      };
    });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const icdParam = selectedICD
        ? `?custom_house=${encodeURIComponent(selectedICD)}`
        : "";
      const res = await axios.get(
        `${apiBase}/report/monthly-containers/${year}/${month}${icdParam}`
      );
      setData(res.data);
    } catch (err) {
  ...
  const sortedData = [...data].sort((a, b) => {
    ...
    switch (sortColumn) {
      ...
      case "container20Ft":
        aValue = Math.max(
          0,
          (Number(a.container20Ft) || 0) - (Number(a.lcl20Ft) || 0)
        );
        bValue = Math.max(
          0,
          (Number(b.container20Ft) || 0) - (Number(b.lcl20Ft) || 0)
        );
        break;
      case "container40Ft":
        aValue = Math.max(
          0,
          (Number(a.container40Ft) || 0) - (Number(a.lcl40Ft) || 0)
        );
        bValue = Math.max(
          0,
          (Number(b.container40Ft) || 0) - (Number(b.lcl40Ft) || 0)
        );
        break;
      case "lcl20Ft":
      ...
      case "teu":
      default:
        const aNet20 = Math.max(
          0,
          (Number(a.container20Ft) || 0) - (Number(a.lcl20Ft) || 0)
        );
        const aNet40 = Math.max(
          0,
          (Number(a.container40Ft) || 0) - (Number(a.lcl40Ft) || 0)
        );
        const bNet20 = Math.max(
          0,
          (Number(b.container20Ft) || 0) - (Number(b.lcl20Ft) || 0)
        );
        const bNet40 = Math.max(
          0,
          (Number(b.container40Ft) || 0) - (Number(b.lcl40Ft) || 0)
        );
  ...
  const totalBEs = data.reduce(
    (sum, row) => sum + (Number(row.beDateCount) || 0),
    0
  );
  const totalNet20Ft = data.reduce(
    (sum, row) =>
      sum +
      Math.max(
        0,
        (Number(row.container20Ft) || 0) - (Number(row.lcl20Ft) || 0)
      ),
    0
  );
  const totalNet40Ft = data.reduce(
    (sum, row) =>
      sum +
      Math.max(
        0,
        (Number(row.container40Ft) || 0) - (Number(row.lcl40Ft) || 0)
      ),
    0
  );
  const totalLCL = data.reduce(
    (sum, row) => sum + (Number(row.lcl20Ft) || 0) + (Number(row.lcl40Ft) || 0),
    0
  );
  const totalTEU = data.reduce((sum, row) => {
    const net20Ft = Math.max(
      0,
      (Number(row.container20Ft) || 0) - (Number(row.lcl20Ft) || 0)
    );
    const net40Ft = Math.max(
      0,
      (Number(row.container40Ft) || 0) - (Number(row.lcl40Ft) || 0)
    );
          ...
          <CardContent>
            ...
            >
              ...
              <Button
                ...
                size="small"
                startIcon={
                  <span role="img" aria-label="View Detailed Report">
                    🗂
                  </span>
                }
                sx={{
                    ...
                    <TableRow sx={{ bgcolor: "grey.50" }}>
                      <TableCell sx={{ fontWeight: "bold", fontSize: "1rem" }}>
                        <TableSortLabel
                          ...
                          direction={
                            sortColumn === "importer" ? sortDirection : "asc"
                          }
                      ...
                      <TableCell sx={{ fontWeight: "bold", fontSize: "1rem" }}>
                        <TableSortLabel
                          ...
                          direction={
                            sortColumn === "beDateCount" ? sortDirection : "asc"
                          }
                      ...
                      <TableCell sx={{ fontWeight: "bold", fontSize: "1rem" }}>
                        <TableSortLabel
                          ...
                          direction={
                            sortColumn === "lcl20Ft" ? sortDirection : "asc"
                          }
                      ...
                      <TableCell sx={{ fontWeight: "bold", fontSize: "1rem" }}>
                        <TableSortLabel
                          ...
                          direction={
                            sortColumn === "lcl40Ft" ? sortDirection : "asc"
                          }
                      ...
                      <TableCell sx={{ fontWeight: "bold", fontSize: "1rem" }}>
                        <TableSortLabel
                          ...
                          direction={
                            sortColumn === "teu" ? sortDirection : "asc"
                          }
                  ...
                  <TableBody>
                    {loading
                      ...
                      : sortedData.map((row, idx) => {
                          const net20Ft = Math.max(
                            0,
                            (Number(row.container20Ft) || 0) -
                              (Number(row.lcl20Ft) || 0)
                          );
                          const net40Ft = Math.max(
                            0,
                            (Number(row.container40Ft) || 0) -
                              (Number(row.lcl40Ft) || 0)
                          );
