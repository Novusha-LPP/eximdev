...
const DoManagementDashboard = () => {
    ...
    const { data: rawData, loading } = useLiveAnalytics('do-management', startDate, endDate, importer, selectedBranch, selectedCategory);
    ...
    // Jobs currently in DO Planning (have do_planning_date, no do_completed)
    const pending = data.summary.in_do_planning || 0;
    ...
    return (
        <div className={`tv-dashboard-wrapper tv-severity-${severity}`}>
            ...
            <div className="tv-center">
                <div className="tv-label">Alvision Pulse — DO </div>
                <div className={`tv-number tv-number-${severity}`}>
                    <AnimatedNumber value={pending} />
                </div>
                <div className="tv-subtitle">DO Planning</div>
                {severity === 'green' && (
                    <motion.div
