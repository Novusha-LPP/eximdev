...
const ESanchitDashboard = () => {
    ...
    const { data: rawData, loading } = useLiveAnalytics('esanchit', startDate, endDate, importer, selectedBranch, selectedCategory);
    ...
    const pending = data.summary.esanchit_pending || 0;
    ...
    return (
        <div className={`tv-dashboard-wrapper tv-severity-${severity}`}>
            ...
            <div className="tv-center">
                <div className="tv-label">Alvision Pulse — E-Sanchit</div>
                <div className={`tv-number tv-number-${severity}`}>
                    <AnimatedNumber value={pending} />
                </div>
                <div className="tv-subtitle">Pending</div>
                {severity === 'green' && (
                    <motion.div
