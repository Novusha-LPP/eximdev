...
function CustomerKycForm() {
  ...
  const formik = useFormik({
    initialValues: {
      category: "Individual/ Proprietary Firm", // Default to match checked radio in HTML if needed, or keeping empty
      name_of_individual: "",
      status: "",
      // Branch addresses
      factory_addresses: [
  ...
  const handleClearConfirmation = () => {
    setDialogState({
      ...
      content: (
        <div>
          <p>
            Are you sure you want to clear the entire form? All entered data will
            be lost.
          </p>
        </div>
      ),
  ...
  const handleRemoveField = (index) => {
    if (formik.values.factory_addresses.length > 1) {
      ...
      formik.setValues({ ...formik.values, factory_addresses: updated });
    }
  };
  ...
  const handleRemoveBank = (index) => {
    if (formik.values.banks.length > 1) {
      ...
      formik.setValues({ ...formik.values, banks: updated });
    }
  };

  const handleRemoveBranch = (index) => {
    const updated = formik.values.branches.filter((_, i) => i !== index);
    formik.setValues({ ...formik.values, branches: updated });
  };
  ...
  const handleSameAsPermanentAddress = (event) => {
    if (event.target.checked) {
      ...
      formik.setValues({ ...formik.values, sameAsPermanentAddress: false });
    }
  };
  ...
  const validateBanks = (banks) => {
    // keeping validation minimal as strict rules removed in previous turn?
    // or just checking array
    return [];
  };
            ...
            <div className="panel">
              ...
              <div className="section">
                ...
                <div id="bank-list">
                  ...
                  {/* Finance Details (Visual/Static Only for now as requested by user HTML) */}
