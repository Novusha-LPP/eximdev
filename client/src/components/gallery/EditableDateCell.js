  ...
  const handleFreeTimeChange = (value) => {
    ...
    axios
      ...
      .then(() => {
        ...
        setNotification({
          open: true,
          message: `Free time updated to ${value} days`,
          severity: "success",
        });
        ...
        if (JSON.stringify(updatedContainers) !== JSON.stringify(containers)) {
          ...
          axios
            ...
            .then(() => {
              setNotification({
                open: true,
                message: "Detention dates updated",
                severity: "info",
              });
            })
            .catch((err) => {
              ...
              setNotification({
                open: true,
                message: "Error updating detention dates",
                severity: "error",
              });
            });
        }
      ...
      .catch((err) => {
        ...
        setNotification({
          open: true,
          message: "Error updating free time",
          severity: "error",
        });
      });
  };
    ...
    <div style={{ display: "flex", gap: "20px" }}>
      ...
      <div>
        {type_of_b_e !== "Ex-Bond" && (
          <>
            <strong>ETA :</strong>{" "}
            {isNaN(new Date(dates.vessel_berthing).getTime())
              ? "N/A"
              : dates.vessel_berthing?.slice(0, 10).replace("T", " ") ||
                "N/A"}{" "}
            <FcCalendar
              style={styles.icon}
