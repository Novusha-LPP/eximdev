...
function EditDoPlanning() {
  ...
  // Fetch empty off locations on component mount
  useEffect(() => {
  ...
  const filteredDoListOptions = [
    { value: "", label: "Select DO List" },
    ...emptyOffLocations
      .filter(loc => loc.active?.toUpperCase() === "YES" && loc.assigned_branches.includes(branch_code))
