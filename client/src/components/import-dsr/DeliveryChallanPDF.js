...
const DeliveryChallanPdf = ({ year, jobNo, branch_code, trade_type, mode, containerIndex = 0, renderAsIcon = false }) => {
    ...
    try {
      ...
      const details = [
        ...
        ...(jobData.consignment_type !== 'LCL' ? [
          ...
          { space: true },
          { label: 'SEAL NO.', value: Array.isArray(container.seal_number) ? (container.seal_number.length > 0 ? container.seal_number[container.seal_number.length - 1] : '') : (container.seal_number || '') },
          { space: true }
        ] : []),
