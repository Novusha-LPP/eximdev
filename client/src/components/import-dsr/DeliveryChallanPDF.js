...
const DeliveryChallanPdf = ({
  ...
  const generateDeliveryChallan = async () => {
    try {
      ...
      pdf.text("SURAJ FORWARDERS PVT LTD", pageWidth / 2, topMargin + 35, {
        align: "center",
      });
      ...
      pdf.text("Regd. Office :", pageWidth / 2, topMargin + 50, {
        align: "center",
      });
      ...
      const address =
        "A – 306, Wall Street – II, Opp. Orient Club, Ellis Bridge, Ahmedabad –380006(India)";
      const addressLines = wrapText(address, contentWidth);
      let addressY = topMargin + 60;
      addressLines.forEach((line, index) => {
        pdf.text(line, pageWidth / 2, addressY + index * 12, {
          align: "center",
        });
      });

      const phone =
        "Phone : (079) 30082020 / 21 / 22  Fax: (079) 26401929  E-mail:surajahd@eth.net";
      ...
      pdf.text("Website : www.surajforwarders.com", pageWidth / 2, websiteY, {
        align: "center",
      });
      ...
      if (
        description.toLowerCase().includes("scrap") ||
        description.toLowerCase().includes("steel")
      ) {
      ...
      pdf.setFontSize(12);
      pdf.text(
        "With reference to the above subject, we are sending herewith the following goods, the",
        leftMargin,
        refY
      );
      pdf.text(
        "details given below for your references.",
        leftMargin,
        refY + 20
      );
      ...
      const container =
        jobData.container_nos && jobData.container_nos[containerIndex]
      ...
      const details = [
        {
          label: "B/E  No. & DATE",
          value: `${jobData.be_no || ""} DATE ${formatDate(jobData.be_date)}`,
        },
        { space: true },
        ...
        {
          label: "INV. NO. & DATE",
          value: `${jobData.invoice_number || ""} Dt. ${formatDate(
            jobData.invoice_date
          )}`,
        },
        { space: true },
        ...
        {
          label: "COMMODITY",
          value: `${jobData.description || ""}`.toUpperCase(),
        },
        { space: true },
        {
          label: "WT. AS PER DOCS",
          value: `${container.container_gross_weight || ""} KGS GROSS WT`,
        },
        { space: true },
      ...
      pdf.setFontSize(12);
      pdf.text(
        "Kindly acknowledge the receipt by returning the duplicate copy duly stamped & signed",
        leftMargin,
        ackY
      );
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          signatureImg.onerror = () => {
            console.warn(
              "Failed to load signature image, continuing without it"
            );
            resolve();
          };

          signatureImg.src =
            "https://exim-images-p1.s3.ap-south-1.amazonaws.com/Scan_Signature/Screenshot_20250602_111546_Samsung+Notes.jpg";
        });
      } catch (error) {
      ...
      const containerNumber =
        container.container_number || `Container${containerIndex + 1}`;
