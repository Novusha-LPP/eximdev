...
export default function EmailConfiguration() {
  ...
  const fetchConfig = () => {
    ...
    setTimeout(() => {
      ...
      const mockConfig = {
        ...
        smtp_port: 587,
        username: "it-helpdesk@company.com",
        password: "********",
        use_tls: true,
        use_ssl: false,
  ...
  const handleTestConfig = () => {
    ...
    setTesting(true);
    setTestLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Starting email test...`]);

    // Simulate test process
    setTimeout(() => {
      setTestLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Connecting to SMTP server: ${config.smtp_server}:${config.smtp_port}`]);
      
      setTimeout(() => {
        setTestLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Authenticating with username: ${config.username}`]);
        
        setTimeout(() => {
          setTestLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] Sending test email to: ${config.test_email}`]);
          
          setTimeout(() => {
            const success = Math.random() > 0.2; // 80% success rate for demo
            if (success) {
              setTestResult("success");
              setTestLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] ✓ Test email sent successfully!`]);
              ...
              setTestResult("error");
              setTestLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] ✗ Failed to send test email. Check credentials and SMTP settings.`]);
