...
import {
    ...
    Grid, Box, CircularProgress, Divider, Typography, Card, CardContent
} from '@mui/material';
...
const KPIReviewerDashboard = () => {
    ...
    const getHelperText = (key) => {
        ...
        switch (key) {
            case 'quantityScore': {
                ...
                return isOverridden 
                    ? `${text} — [HOD Override Active: ${currentScore.toFixed(1)}]` 
                    : text;
            }
            case 'qualityScore': {
    ...
    const handleAction = async (sheetId, action) => {
        setMetaData(null);
        setReviewDialog({
            open: true,
            sheetId,
        ...
        if (action === 'CHECK') {
            ...
            // find sheet to get employeeId, year, month
            let sheet = null;
            // Search in pending check list
            if (data.pending_check) {
                sheet = data.pending_check.find(s => s._id === sheetId);
            }
            if (!sheet && data.pending_verify) {
                sheet = data.pending_verify.find(s => s._id === sheetId);
            }
            if (!sheet && data.pending_approve) {
                sheet = data.pending_approve.find(s => s._id === sheetId);
            }
            if (!sheet && teamHistory) {
                sheet = teamHistory.find(s => s._id === sheetId);
            }
        ...
        >
            ...
            <Dialog open={reviewDialog.open} onClose={() => { setReviewDialog({ ...reviewDialog, open: false }); setMetaData(null); }} fullWidth maxWidth={reviewDialog.action === 'CHECK' ? "md" : "sm"}>
                <DialogTitle style={{ fontWeight: 700, color: '#1e293b' }}>
                    {reviewDialog.action === 'CHECK' ? '✓ HOD Check & Evaluate Sheet' :
                    ...
                    {reviewDialog.action === 'CHECK' && loadingScores ? (
                        ...
                        <>
                            ...
                            <TextField
                                ...
                                placeholder={reviewDialog.action === 'REJECT' ? "Please explain why this sheet is being rejected..." : "Add HOD evaluation feedback comments..."}
