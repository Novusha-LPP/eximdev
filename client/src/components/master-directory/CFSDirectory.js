...
const CFSDirectory = () => {
  ...
  const fetchCfsList = async () => {
    try {
      ...
      handleSnackbar("Failed to fetch CFS list", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      handleSnackbar(error.response?.data?.message || "Failed to save CFS", "error");
    }
  };
  ...
  const filteredList = cfsList.filter((l) =>
