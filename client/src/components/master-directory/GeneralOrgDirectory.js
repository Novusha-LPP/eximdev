...
const GeneralOrgDirectory = () => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        if (branch._id && editingId) {
          try {
            await axios.delete(`${process.env.REACT_APP_API_STRING}/delete-general-org/${editingId}/branch/${branch._id}`);
            handleSnackbar("Branch deleted from database successfully", "success");
            fetchGeneralOrgs(); // Refresh the main list
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        if (account._id && branch._id && editingId) {
          try {
            await axios.delete(`${process.env.REACT_APP_API_STRING}/delete-general-org/${editingId}/branch/${branch._id}/account/${account._id}`);
  ...
  const fetchGeneralOrgs = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${process.env.REACT_APP_API_STRING}/get-general-orgs`);
      setGeneralOrgs(res.data);
    } catch (error) {
      console.error("Error fetching general orgs:", error);
      handleSnackbar("Failed to fetch general orgs", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await axios.put(`${process.env.REACT_APP_API_STRING}/update-general-org/${editingId}`, formData);
        handleSnackbar("General Org updated successfully", "success");
      } else {
        await axios.post(`${process.env.REACT_APP_API_STRING}/add-general-org`, formData);
        handleSnackbar("General Org added successfully", "success");
      }
      handleClose();
      fetchGeneralOrgs();
    } catch (error) {
      handleSnackbar(error.response?.data?.message || "Failed to save general org", "error");
    }
  };
  ...
  const handleDelete = (id) => {
    setConfirmDialog({
      open: true,
      title: "Delete General Org",
      message: "Are you sure you want to delete this general org? This action cannot be undone.",
      onConfirm: async () => {
        try {
          await axios.delete(`${process.env.REACT_APP_API_STRING}/delete-general-org/${id}`);
          handleSnackbar("General Org deleted successfully", "success");
          fetchGeneralOrgs();
        } catch (error) {
          handleSnackbar("Failed to delete general org", "error");
        }
      }
    });
  };
  ...
  const filteredLines = generalOrgs.filter((l) =>
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">General Orgs</Typography>
      </Breadcrumbs>

      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">General Orgs Directory</Typography>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
        <DialogTitle>{editingId ? "Edit General Org" : "Add New General Org"}</DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, pt: 1 }}>
            <TextField
              fullWidth
              label="General Org Name"
              value={formData.name}
...
export default GeneralOrgDirectory;
