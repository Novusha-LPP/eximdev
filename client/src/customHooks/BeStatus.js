...
const BEStatus = ({ isOpen, onClose, beNo, beDt, location }) => {
  ...
  const fetchBEDetails = async () => {
    ...
    try {
      ...
      if (err.code === 'ERR_NETWORK') {
        setError('Cannot connect to backend server. Ensure the proxy server is running on port 5000.');
  ...
  const renderTabContent = () => {
    if (!beDetails) return <Typography variant="body2">No data available.</Typography>;

    if (activeTab === 0) {
      const d = beDetails.beDetailsModel?.[0] || {};
      const fields = [
        { label: 'IEC', value: d.iec, icon: <FormatListBulletedIcon fontSize="small" sx={{ fontSize: '0.9rem' }} /> },
        ...
        { label: 'Gross Weight', value: d.grossWeight ? `${d.grossWeight} Kg` : d.grossWeight },
        { label: 'Total Duty', value: d.totalDuty },
        { label: 'Fine & Penalty', value: d.finePenalty },
        { label: 'WBE Number', value: d.wbeNo }
      ];
      return <KeyValuePanel title="Bill of Entry Information" fields={fields} />;
    }
    ...
    if (activeTab === 2) {
      ...
      const fields = [
        { label: 'Challan Number', value: p.challaNo, icon: <PrintIcon fontSize="small" sx={{ fontSize: '0.9rem' }} /> },
        ...
        <Paper elevation={0} sx={{ borderRadius: 1, border: '1px solid', borderColor: 'divider' }}>
          ...
          <TableContainer sx={{ maxHeight: 360 }}>
            <Table size="small" stickyHeader>
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700 }}>Document Version</TableCell>
                  <TableCell sx={{ fontWeight: 700 }}>Description</TableCell>
                  <TableCell sx={{ fontWeight: 700, textAlign: 'center' }}>Validity</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {rows.map((doc, i) => (
                  <TableRow key={i}>
                    <TableCell>
                      <Chip label={renderValue(doc.docVersion)} size="small" variant="outlined" sx={{ fontWeight: 600 }} />
                    </TableCell>
                    <TableCell>
                      <Typography variant="body2" color="text.secondary">
            ...
            <Table size="small" stickyHeader>
              ...
              <TableBody>
                {rows.map((r, i) => (
                  <TableRow key={i}>
                    <TableCell>
                      <Chip label={renderValue(r.amendmentNo)} size="small" variant="outlined" sx={{ fontWeight: 600 }} />
                    </TableCell>
                    <TableCell sx={{ fontWeight: 500 }}>{renderValue(r.amendmentDate)}</TableCell>
                    <TableCell>
                      <Chip label={renderValue(r.amendmentType)} size="small" variant="outlined" />
                    </TableCell>
                    <TableCell sx={{ fontWeight: 500 }}>{renderValue(r.status)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
    ...
    if (activeTab === 5) {
      ...
      const AgencyTable = ({ title, rows }) => (
        <Paper elevation={0} sx={{ flex: 1, borderRadius: 1, border: '1px solid', borderColor: 'divider' }}>
          <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>
              {title}
            </Typography>
          ...
          <TableContainer sx={{ maxHeight: 300 }}>
            <Table size="small" stickyHeader>
              ...
              <TableBody>
                {rows.map((row, idx) => (
                  <TableRow key={idx}>
                    <TableCell sx={{ fontWeight: 500 }}>{renderValue(row.agencyName)}</TableCell>
                    <TableCell sx={{ textAlign: 'center' }}>{renderValue(row.status)}</TableCell>
                    <TableCell sx={{ fontWeight: 500 }}>{renderValue(row.date)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
      ...
      return (
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }}>
          ...
          {others.length > 0 && <AgencyTable title="Other Agency Status" rows={others} />}
        </Box>
      );
    }
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                <Chip label={renderValue(beNo)} size="small" variant="outlined" sx={{ fontWeight: 600 }} />
                <Chip
