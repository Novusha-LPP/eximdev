...
function useFetchLrJobList(status, searchQuery) {
  ...
  const fetchJobs = useCallback(
    async (page) => {
      ...
      try {
        ...
        // Construct API URL
        const apiUrl = `${process.env.REACT_APP_API_STRING}/lr-job-list?status=${status}&search=${formattedSearchQuery}&page=${page}&limit=100`;
