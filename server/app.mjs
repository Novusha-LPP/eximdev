...
const MONGODB_URI_RAW = process.env.NODE_ENV === "production"
  ? process.env.PROD_MONGODB_URI
  : process.env.NODE_ENV === "server"
    ? process.env.SERVER_MONGODB_URI
    : process.env.DEV_MONGODB_URI;

import { cleanUri } from "./utils/uriCleaner.mjs";
const MONGODB_URI = cleanUri(MONGODB_URI_RAW);

// Log the URI for debugging (mask password)
const maskedURI = MONGODB_URI ? MONGODB_URI.replace(/\/\/.*:.*@/, "//USER:PASS@") : "NOT SET";
console.log("[DEBUG] MongoDB URI (masked):", maskedURI);
