...
import getAllDoJobs from "./routes/import-do/getAllDoJobs.mjs";
...
app.get("/", async (req, res) => {
  try {
    ...
    /* 
    // JobModel is missing in imports. Commenting out to prevent crash in tests if not used. 
    // If this route is hit, it might fail if JobModel is not global.
...
if (!disableCluster && cluster.isPrimary) {
  ...
  if (process.env.NODE_ENV !== 'test') {
    ...
    const closeMongo = async () => {
      // ... logic ...
      try {
        // Note: server variable is local to .then scope in previous code.
        // But here we need access.
        // Actually this logic was at the end.
