...
const attendanceAuthBridge = async (req, res, next) => {
  ...
  try {
    ...
    const freshUser = await UserModel.findById(verified._id)
      .select('_id username first_name last_name role company_id department_id shift_id current_status last_punch_date last_punch_type employment_type gender leave_settings')
      .lean();
