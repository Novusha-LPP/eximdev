...
const attendanceAuthBridge = async (req, res, next) => {
  ...
  try {
    ...
    const freshUser = await UserModel.findById(verified._id)
      .select('_id username first_name last_name role company company_id department_id shift_id weekoff_policy_id holiday_policy_id attendance_settings current_status last_punch_date last_punch_type employment_type gender leave_settings isAttendanceAllowedAdmin is_operator category')
      .lean();
