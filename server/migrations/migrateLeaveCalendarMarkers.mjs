...
// By default, do a dry-run for safety unless --apply is provided
const DRY_RUN = !hasFlag('--apply');
const CREATE_MISSING = hasFlag('--create-missing') || hasFlag('--apply'); // Default to create missing when applying
const OVERWRITE_EMPTY = hasFlag('--overwrite-empty') || hasFlag('--apply'); // Default to overwrite empty when applying
const TZ_OVERRIDE = getArg('--tz', '');
const MONGO_URI = getArg('--uri', '') || 
...
const SHOW_TABLE = hasFlag('--table') || hasFlag('--summary') || !hasFlag('--quiet');
const QUIET = hasFlag('--quiet');

// User requested targeting approved leaves
const STATUSES = getArg('--statuses', 'approved')
  .split(',')
...
async function main() {
  if (!MONGO_URI) {
    throw new Error('MongoDB URI not specified. Please set PROD_MONGODB_URI or MONGO_URI in .env or pass as --uri=...');
  }
  ...
  if (!QUIET) {
    ...
    console.log(`Mode: ${DRY_RUN ? 'DRY RUN ONLY (Pass --apply to execute changes)' : 'LIVE EXECUTION'}`);
    console.log(`Statuses target: ${STATUSES.join(', ')}`);
  }
