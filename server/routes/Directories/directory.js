...
router.get('/api/directory/stats', async (req, res) => {
  try {
    ...
    const approved = await Directory.countDocuments({ approvalStatus: 'Approved' });
    const pending = await Directory.countDocuments({ approvalStatus: 'Pending' });
    const rejected = await Directory.countDocuments({ approvalStatus: 'Rejected' });
...
router.post('/api/directory/', validateDirectory, async (req, res) => {
  try {
