...
router.post("/api/jobs/add-job-exp-man", 
  ...
  try {
    ...
    const yearFormat = year || `${currentYear.toString().slice(-2)}-${(currentYear + 1).toString().slice(-2)}`;
    ...
    if (job_no && job_no.length > 0) {
      ...
      const jobCount = await ExportJobModel.countDocuments({ year: yearFormat });
      const nextSequence = (jobCount + 1).toString().padStart(5, "0");
      newJobNo = `${nextSequence}`;
    }
