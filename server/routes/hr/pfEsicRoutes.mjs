...
router.get("/api/hr/pf-esic/:userId", authMiddleware, async (req, res) => {
    try {
        ...
        // Fetch fresh user data from DB to verify role, department, and modules
        const requester = await UserModel.findById(req.user._id).select('role department modules');
        ...
        const employee = await UserModel.findById(userId).select("pf_no pf_card_url esic_no esic_card_url username first_name last_name");
        if (!employee) {
