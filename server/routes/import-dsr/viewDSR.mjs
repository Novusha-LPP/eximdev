...
router.get("/api/dsr/:year", async (req, res) => {
  try {
    ...
    const jobs = await JobModel.find(query).select(
      "job_number job_no importer job_date supplier_exporter invoice_number invoice_date awb_bl_no awb_bl_date hawb_hbl_no hawb_hbl_date mbl_details hbl_details commodity no_of_pkgs net_weight loading_port free_time container_nos transporter remarks detailed_status do_completed do_validity"
    );
