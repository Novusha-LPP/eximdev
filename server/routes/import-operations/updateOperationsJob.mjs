...
const extractJobInfo = async (req, res, next) => {
  try {
    const { mode, year, job_no } = req.params;
    
    // Find the job to get its document ID
    const job = await JobModel.findOne({ mode: mode.toUpperCase(), job_no, year }).lean();
    if (job) {
      req.jobInfo = {
        documentId: job._id,
        job_no,
...
router.patch("/api/update-operations-job/:mode/:year/:job_no", extractJobInfo, auditMiddleware("Job"), async (req, res) => {
  const { mode, year, job_no } = req.params;
  ...
  try {
    ...
    if (recomputedStatus && recomputedStatus !== job.detailed_status) {
      job = await JobModel.findOneAndUpdate(
        { mode: mode.toUpperCase(), year, job_no },
