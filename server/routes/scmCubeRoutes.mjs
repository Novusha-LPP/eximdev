  ...
  try {
    ...
    const responseData = {
      ...
      BE_Details: {
        ...
        "Whether Prior BE": validateChar(job.be_filing_type === "Prior" ? "A" : "N", 1, true, "Whether Prior BE"),
      ...
      SupportingDocumentList: [
        ...(job.documents || []).map(doc => ({
          ...
          DocumentFilePath: validateChar(Array.isArray(doc.url) ? doc.url[0] : doc.url, 200, true, "DocumentFilePath"),
    ...
    if (responseData.SupportingDocumentList.length === 0) {
      responseData.SupportingDocumentList.push({
        DocumentCode: validateChar("", 8, false),
        DocumentName: validateChar("", 50, true),
        DocumentFilePath: validateChar("", 200, true),
        DocumentFileFormat: validateChar("PDF", 10, true)
      });
    }
