...
async function checkGIMConsignment() {
    try {
        ...
        console.log('--- Consignment Type Breakdown for GIM ---');
        ...
        const counts = await JobModel.aggregate([
            ...
            { $group: { _id: '$consignment_type', count: { $sum: 1 } } }
        ]);

        counts.forEach(c => console.log(`${c._id || 'NO_TYPE'}: ${c.count}`));
