...
const loginUrl = "http://0.0.0.0:9006/api/login";
const reportUrl = "http://0.0.0.0:9006/api/attendance/admin-report";
...
async function main() {
    try {
        ...
        const companiesRes = await axios.get("http://0.0.0.0:9006/api/attendance/master/companies", {
        ...
        const usersCountRes = await axios.get("http://0.0.0.0:9006/api/attendance/debug/users-by-company", {
            headers: { Cookie: cookie }
        }).catch(() => null);
        ...
        if (reportRes.data.data?.length > 0) {
            ...
            const allUsersRes = await axios.get("http://0.0.0.0:9006/api/attendance/admin-report?startDate=2026-02-01&endDate=2026-03-31&departmentId=all", {
                headers: { Cookie: cookie }
            });
