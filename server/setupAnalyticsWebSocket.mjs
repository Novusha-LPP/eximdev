    ...
    wss.on('connection', (ws) => {
        ws.on('message', async (message) => {
            try {
                ...
                const { module, startDate, endDate, importer } = payload;
                connections.set(ws, { module, startDate, endDate, importer });
        ...
        const intervalId = setInterval(async () => {
            ...
            if (ws.readyState === ws.OPEN && meta && meta.module) {
                try {
                    const data = await fetchAnalyticsData(meta.module, meta.startDate, meta.endDate, meta.importer);
